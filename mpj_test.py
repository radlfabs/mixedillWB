...
sys.path.append('../mixedillWB/src')
sys.path.append(join("mixedillWB", "src"))
import wb_net
import ops
import dataset
...
import weight_refinement as weight_refinement
...
if __name__ == '__main__':
    ...
    result = test_net(net=net, device=device, data_dir=args.tedir,
    ...
    path = os.path.join(os.getcwd(), "images", "tiff", "bayerRG12_bothIll.tiff")
    img_raw = cv2.imread(path, -1) # -1 for uint16
    ...
    camera = Camera()
    camera.bitdepth = 12
    # camera.rgbGain = [1, 1, 1]

    camera.makeNodes(
        DebayeringNode, 
        # ManualWhitebalanceNode,
        whitebalOnoNode,
        # whitebalPRNode,
        ColorMatrixNode, 
        GammaCorrectionNode,
        # AveragePolarizationImagesNode
        )

    img = camera.render(img_raw)
